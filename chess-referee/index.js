...
export class GameRoom extends DurableObject {
  ...
  async webSocketMessage(ws, raw) {
    ...
    if (msg.type === "race") {
      // Relay boat-race messages verbatim to the other player. The referee
      // does not interpret these (no turn order, no board state) — it simply
      // forwards the challenge / countdown / position / finish packets so the
      // two clients can run a synchronized race over the same connection.
      this._broadcast({ ...msg, by: color }, ws);
      return;
    }
  }
